...
/* Scale 12 bits to 0-10000 */
#define ADC_SCALING 1.0 / 0.4095
...
void system_adc_worker(void)
{
        ...
        while(!chThdShouldTerminateX()) {
                ...
                /* broadcast data over serial port at 1/10 the CAN sample rate */
                if (counter % 10 == 0) {
                        log_info(",%d,%d,%d,%d,%s\r\n", analog_sample.data16[0], analog_sample.data16[1], analog_sample.data16[2], analog_sample.data16[3], get_test_buffer());
                }
                log_debug("Sample ADC %d, %d, %d, %d, %s\r\n", analog_sample.data16[0], analog_sample.data16[1], analog_sample.data16[2], analog_sample.data16[3], get_test_buffer());
